...
document.addEventListener('DOMContentLoaded', (event) => {
	...
	horizontalScrollingModePDF();
		...
		if (viewHeader.textContent.indexOf('Displaying') !== -1) {
			...
			// Not showing results, so hide the sorting options
			console.log("Search results not found, removed sorting options");
			formElements.forEach(function (element) {
				element.style.display = 'none'; // Reset to default or use 'block', 'flex', etc., as appropriate
			});
		}
		...
		const dropdown = container.querySelector('.species-select') || container.querySelector('.strain-select');

		
		list.style.display = 'none'; // Optionally hide the &amp;amp;amp;lt;ul&amp;amp;amp;gt; list
		dropdown.add(new Option("Select Option", "", true, true)); // Placeholder option
